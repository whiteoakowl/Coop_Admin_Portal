...
test('Floater Teams: 4 hour cards, rank/remove start locked, hour title is editable', async (t) => {
  ...
  const section = db.prepare('SELECT id, position FROM volunteer_sections WHERE volunteer_list_id = ? ORDER BY position LIMIT 1').get(list.id);
  db.prepare("INSERT INTO volunteer_members (volunteer_list_id, section_id, member_id, rank) VALUES (?, ?, ?, 'sometimes')").run(list.id, section.id, memberId);
...
test('POST /admin/volunteers/:day/teams/:sectionId/hour-label renames only that one hour', async (t) => {
  ...
  const list = db.prepare("SELECT id FROM volunteer_lists WHERE day = 'monday'").get();
  const sections = db.prepare('SELECT id, position FROM volunteer_sections WHERE volunteer_list_id = ? ORDER BY position').all(list.id);
  ...
  await t.test('renaming hour 1 does not reset the other hours to their defaults', async () => {
    ...
    db.prepare("INSERT INTO class_schedule_hours (day, position, label) VALUES ('monday', 2, 'Custom Hour 2') ON CONFLICT(day, position) DO UPDATE SET label = excluded.label").run();
    ...
    const hour1 = db.prepare("SELECT label FROM class_schedule_hours WHERE day = 'monday' AND position = ?").get(firstSection.position);
    assert.equal(hour1.label, 'Morning Shift');

    const hour2 = db.prepare("SELECT label FROM class_schedule_hours WHERE day = 'monday' AND position = 2").get();
