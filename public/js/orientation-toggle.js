...
(function () {
  document.addEventListener('click', async (e) => {
    ...
    if (dot) {
      ...
      const semesterId = dot.getAttribute('data-semester-id') || '';
      const field = dot.getAttribute('data-field');
      const nextValue = dot.getAttribute('data-complete') !== '1';

      dot.disabled = true;
      try {
        const res = await fetch(`/admin/orientation/${memberId}/toggle`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-CSRF-Token': window.CSRF_TOKEN || '' },
          body: new URLSearchParams({ field, value: nextValue ? '1' : '0', semesterId }).toString(),
        });
        ...
        if (row) {
          ...
          const percentCell = row.querySelector('td:nth-last-child(2)');
    ...
    // A real request: "Members are only listed once with arrow dropdown
    // for children to expand and close" - the children row is the very
    // next sibling <tr> in the table (views/admin-orientation.ejs).
    const expandBtn = e.target.closest('[data-orientation-expand]');
    if (expandBtn) {
      const row = expandBtn.closest('tr');
      const childrenRow = row ? row.nextElementSibling : null;
      if (!childrenRow || !childrenRow.classList.contains('orientation-children-row')) return;
      const nowExpanded = childrenRow.hidden;
      childrenRow.hidden = !nowExpanded;
      expandBtn.setAttribute('aria-expanded', String(nowExpanded));
      expandBtn.classList.toggle('orientation-expand-open', nowExpanded);
    }
  });
})();
